...
{
    // Temporarily using mock data until Firestore is properly seeded
    // This ensures the website works immediately without database setup
    ...
    public async Task<List<LLMModel>> GetAllUniqueModelsAsync()
    {
        ...
        all.AddRange(GetMockOverallModels());
        all.AddRange(GetMockCodingModels());
        all.AddRange(GetMockOpenSourceModels());
        return all.GroupBy(m => m.Name).Select(g => g.First()).ToList();
    }

    // Mock data - will be replaced with Firestore data once seeded
